...
class BookingResponseDtoTest {
	...
	void testBookingDetailsResponse() {
		// Arrange
		BookingDetailsResponse response = BookingDetailsResponse.builder().bookingId("BK-DET-1").customerId("C1")
				.serviceId("S1").serviceName("SName").categoryId("Cat1").categoryName("CatName")
				.scheduledDate(LocalDate.now()).timeSlot(TimeSlot.SLOT_16_18).serviceAddress(new Address())
				.issueDescription("Desc").paymentMode(PaymentMode.CASH).status(BookingStatus.CONFIRMED)
		...
		BookingDetailsResponse copy = new BookingDetailsResponse("BK-DET-1", "C1", "S1", "SName", "Cat1", "CatName",
				response.getScheduledDate(), TimeSlot.SLOT_16_18, response.getServiceAddress(), "Desc",
				PaymentMode.CASH, BookingStatus.CONFIRMED, response.getCreatedAt());
	...
	void testBookingListResponse() {
		// Arrange
		BookingListResponse response = BookingListResponse.builder().bookingId("BK-LIST-1").customerId("C1")
				.serviceId("S1").serviceName("SName").categoryId("Cat1").categoryName("CatName")
				.scheduledDate(LocalDate.now()).timeSlot(TimeSlot.SLOT_16_18).serviceAddress(new Address())
				.status(BookingStatus.COMPLETED).createdAt(LocalDateTime.now()).build();
		...
		// Note: Equals depends on all fields, so unless all are null/equal, they won't
		// match.
		// Simple check:
	...
	void testBookingResponse() {
		// Arrange
		BookingResponse response = BookingResponse.builder().bookingId("BK-RES-1").status("SUCCESS").build();
